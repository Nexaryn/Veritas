...
if not os.path.exists(HISTORY_FILE):
    ...
    with open(HISTORY_FILE, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Time', 'News Snippet', 'AI Verdict', 'Confidence', 'User Feedback'])
...
def predict():
    ...
    try:
        ...
        # --- WRITE TO AUDIT TRAIL CSV ---
        with open(HISTORY_FILE, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            
            # We use result.get() to grab the FACT-CHECKED final verdict if it exists, 
            # otherwise it falls back to the raw ML prediction.
            ...
            # Clean up the text snippet to avoid messy CSV formatting
...
def feedback():
    """Handle user feedback for the Decision Support System"""
    data = request.get_json()
    user_thought = data.get('feedback') 
    print(f"📢 USER FEEDBACK RECEIVED: The user says the AI was {user_thought}!")
    
    return jsonify({"success": True, "message": "Thank you for helping me learn!"})

@app.route('/history')
def show_history():
    """Render the audit trail history in JSON format for the MIS admin view"""
    rows = []
